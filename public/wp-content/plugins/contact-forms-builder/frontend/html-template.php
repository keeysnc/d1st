...
function wpdevart_template($atts) {
	...
	if(isset($atts['id']) && $atts['id']!= '') {
		...
		if($form_id == $confirm_form_id) 
		{
			...
			//	By default don't show contact form name on frontend 
			$show_form_name = "no";
			...
			if(!isset($atts['status']) && isset($atts['role'])) {
				...
				if( ($status == 'private' && (!isset($atts['role']) || empty($atts['role']))) ) {
					...
					if(empty($error_msg)) {
						...
						// Include styles and scripts for contact form
										...
										if($form_field->fieldtype == "recaptcha") {
											  ...
											  <div class="wpdevart-input-field field-captcha" style="margin-bottom:20px;">
												...
												<div class="input-text input-field-inner">
													<input type="text"  name="submitedRecaptchaValue[]" placeholder="<?php echo stripslashes_deep(esc_attr($form_field->placeholder));?>" <?php if($form_field->is_required == 1) echo "required"; ?> />
